...
def validate_xml(xml_str: str) -> list[tuple[str, bool, str]]:
    ...
    # 3. MessageHeader — all required fields
    hdr_fields = ["TransmittingCountry", "ReceivingCountry", "MessageType",
                  "MessageRefID", "MessageTypeIndic", "ReportingPeriod", "Timestamp"]
    missing_hdr = [f for f in hdr_fields if text(f"g:MessageHeader/g:{f}") is None]
    check("MessageHeader — all required fields", not missing_hdr,
    ...
    name = text("g:GloBE_Body/g:FilingInfo/g:FilingCE/g:ID/g:Name")
    ...
    # 7. TIN — not placeholder, has required attributes
    tin_el = root.find("g:GloBE_Body/g:FilingInfo/g:FilingCE/g:ID/g:TIN", g)
...
with st.expander("Advanced options"):
    ...
    globe_status = st.selectbox("GloBE status", ["GIR301", "GIR302", "GIR303"],
                                help="GIR301 = Constituent Entity")
    rules       = st.selectbox("Rules", ["GIR204", "GIR201", "GIR202", "GIR203"],
                               help="GIR204 = QDMTT applicable")
